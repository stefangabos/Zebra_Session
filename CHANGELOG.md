...
- **the PHP version declared in `composer.json` is now 7.4**, up from 5.5.2; the library itself is unchanged and still tested on 5.6.40 - the thing that changed is what `Composer` will install on. 7.4 is the oldest PHP the test suite runs on, so it is the oldest version the library is verified against; **if you are on PHP older than 7.4**, install by downloading the release instead of through Composer - nothing about the library has changed for you
- the documented requirement is now PHP 5.6.40, up from 5.5.2; the library itself is unchanged, but 5.6.40 is the version it is actually linted and smoke-tested on, and the static check reads the source against 5.6 as well. if you are on 5.5, the library will in all likelihood keep working exactly as it did, it is simply no longer a version anything here checks
- working further back than 7.4 is checked statically by `composer check-compat-legacy`, which reads the source against 5.6, and by running the library on a real 5.6.40 in `Docker`: `tests/run-legacy.sh` builds a `php:5.6-cli` image, brings up a `MySQL 5.7` container of its own on a private network, lints everything that ships and runs a smoke script that round-trips one session against it. `MySQL 5.7` because PHP 5.6's client does not know the `utf8mb4_0900_ai_ci` collation that MySQL 8 announces. nothing has to be installed locally beyond `Docker`, and `tests/run-legacy.sh --lint` runs the lint on its own, which needs no database at all
- the test suite was overhauled: it runs on PHP 7.4 through 8.4 in continuous integration, every test against both a `PDO` and a `mysqli` connection, and the tests that guard past fixes are themselves checked by reverting each fix and confirming that the test naming it fails
...
- updated table structure to match latest recommended values for MySQL/MariaDB - the new table definition now uses `utf8mb4`/`utf8mb4_unicode_ci` instead of `utf8`, which means new installations require MySQL 5.5.3+ (the requirements in the README have been updated accordingly); existing tables do not need converting - session data is stored in a binary column, so the charset only ever applied to the session id and hash columns
...
- fixed a bug where locking sessions to the visitor's IP address would raise a warning if `$_SERVER['REMOTE_ADDR']` was not set - which is the case for scripts run from the command line and for some SAPI configurations
